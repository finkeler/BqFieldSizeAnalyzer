...
class SchemaField(object):
    ...
    """
    def __init__(self, name, name_full, field_type, parent=None, mode='NULLABLE',
                 description=None, fields=()):
        self._name = name #can be optimized? via name_full
        self._name_full = name_full
        self._parent_full = parent# + '.' if parent is not None else name
        self._parent_short = parent.split('.')[-1] if parent is not None else None
    ...
    @classmethod
    def from_api_repr(cls, api_repr, __parent=None):
        ...
        name = api_repr['name']
        #parent_full = __parent + '.' + name if __parent is not None else None
        ...
        return cls(
            ...
            name=name,
            name_full = name_full,
            fields=[cls.from_api_repr(f, name_full) for f in fields],
            mode=mode.upper(),
